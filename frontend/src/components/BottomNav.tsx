      ...
      <div className="flex items-center justify-between gap-2">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex-1 rounded-3xl px-3 py-3 text-center text-[11px] font-semibold transition-all duration-300 ${
                isActive
          ...
          >
            <span className="block text-xl leading-none">{item.to === '/' ? '🏠' : item.to === '/summon' ? '✨' : item.to === '/party' ? '🛡️' : item.to === '/tower' ? '🏯' : '👤'}</span>
